...
import { X as CloseIcon, Info, Calendar, DollarSign, MapPin, FileText, Image } from 'lucide-react'; // Import icons
...
const Deskripsi = ({ data }) => {
  ...
  return (
    <div className="flex flex-col w-full bg-white rounded-xl shadow-lg p-6 sm:p-8"> {/* Main card container */}
      {/* Arisan Details Section */}
      <h2 className="font-bold text-2xl text-gray-800 mb-6 border-b pb-4 border-gray-100">Detail Arisan</h2>
      ...
      <div className="space-y-4 mb-8"> {/* Increased spacing */}
        <div className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
          <span className="flex items-center text-gray-600 font-medium text-base">
            <Info className="w-5 h-5 mr-2 text-purple-600" /> Title
          </span>
          <span className="text-right text-gray-800 font-semibold text-base break-words max-w-[60%]">{data.title}</span>
        </div>

        <div className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
          <span className="flex items-center text-gray-600 font-medium text-base">
            <MapPin className="w-5 h-5 mr-2 text-purple-600" /> Keterangan
          </span>
        ...
        <div className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
          <span className="flex items-center text-gray-600 font-medium text-base">
            <DollarSign className="w-5 h-5 mr-2 text-purple-600" /> Target
          </span>
        ...
        <div className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
          <span className="flex items-center text-gray-600 font-medium text-base">
            <DollarSign className="w-5 h-5 mr-2 text-purple-600" /> Iuran Bulanan
          </span>
        ...
        <div className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
          <span className="flex items-center text-gray-600 font-medium text-base">
            <Calendar className="w-5 h-5 mr-2 text-purple-600" /> Durasi
          </span>
          <span className="text-right text-gray-800 font-semibold text-base">{data.totalSlot.toLocaleString('id-ID')} Bulan</span>
        </div>
      </div>
      ...
      <p className="text-gray-700 leading-relaxed text-base mb-8 whitespace-pre-line">
        {data.desc || 'Tidak ada deskripsi tersedia untuk arisan ini.'}
      </p>
