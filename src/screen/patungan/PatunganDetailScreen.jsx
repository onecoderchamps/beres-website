...
import {
  ...
  FaTimes,
  FaPlus,
} from "react-icons/fa"; // Added icons for design
...
export default function PatunganDetail() {
  ...
  return (
    <div className="bg-white-50 min-h-screen p-2">
      {" "}
      {/* Soft background, padding-bottom */}
      <BackButton title={"Detail Patungan"} />
      {/* Banner Section */}
      {activeTab !== "Chat" &&
        detailData.banner &&
        detailData.banner.length > 0 && (
          <div className="relative w-full overflow-hidden mb-6">
            {" "}
            {/* Unified container for banner */}
      ...
      {/* Tabs Section */}
      <div className="bg-white rounded-xl shadow-md mx-4 p-1 flex justify-around">
        {" "}
        {/* Card-like tabs */}
        {["Deskripsi", "Syarat", "Perkembangan"].map((tab) => {
          // Conditional rendering for 'Member' tab based on membership status
          if (!detailData?.statusMember?.isMembership && tab === "Perkembangan")
            return null;
      ...
      <div className="p-4 mx-auto max-w-4xl">
        {" "}
        {/* Standard padding and max-width for content */}
        {activeTab === "Deskripsi" && detailData && (
          <Deskripsi data={detailData} />
        )}
        {activeTab === "Syarat" && detailData && <Syarat data={detailData} />}
        {activeTab === "Perkembangan" && detailData && (
          <Member data={detailData} getPatunganDatabase={getPatunganData} />
        )}
              ...
              <>
                ...
                <div className="mb-6">
                  ...
                  <div className="flex items-center justify-center space-x-3">
                    <button
                      onClick={() =>
                        setJumlahLot((prev) => Math.max(1, prev - 1))
                      }
                    ...
                    <input
                      ...
                      value={jumlahLot}
                      onChange={(e) =>
                        setJumlahLot(Math.max(1, parseInt(e.target.value) || 1))
                      }
