...
import {
  ...
  MdAttachMoney,
  MdCheckCircle,
} from "react-icons/md"; // Added new icons
...
const MembershipCard = () => {
  ...
  const getBalance = async () => {
    // Renamed from getDatabase for clarity
    setLoading(true);
    try {
  ...
  const handleTransfer = async () => {
    ...
    if (!formattedPhone.startsWith("+62") || formattedPhone.length < 10) {
      setModalError(
        "Format nomor ponsel tujuan tidak valid. Gunakan format 08xx atau +628xx."
      );
      return;
    }
    ...
    <div className="bg-white rounded-2xl shadow-lg p-6 transform transition-all duration-300 hover:shadow-xl">
      <div className="flex items-start justify-between">
        ...
        <div className="flex-1 cursor-pointer select-none" onClick={getBalance}>
          ...
          {loading ? (
            <p className="text-black text-2xl font-bold mt-2 animate-pulse">
              Rp ...
            </p>
          ) : (
          ...
          {data.role === "2" && (
            ...
            >
              <div className="bg-gray-100 p-2 rounded-full group-hover:bg-yellow-100 transition-colors duration-200">
                <MdSwapHoriz
                  size={24}
                  className="text-gray-600 group-hover:text-yellow-700"
                />
              </div>
              <span className="text-xs font-semibold mt-2 group-hover:text-yellow-700">
                Transfer
              </span>
            </div>
          )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 flex justify-center items-center p-4 z-[100]">
          {" "}
          {/* Higher z-index */}
          <div className="bg-white rounded-xl p-6 w-full max-w-sm shadow-2xl relative animate-scale-in">
            {" "}
            ...
            {transferSuccess ? (
              <div className="flex flex-col items-center text-green-600 py-6">
                ...
                <p className="text-sm text-gray-600 mt-2 text-center">
                  Dana sebesar Rp {formatCurrency(nominal)} berhasil ditransfer
                  ke {tujuan}.
                </p>
              </div>
            ) : (
              <>
                <div className="mb-4">
                  <label htmlFor="tujuanPhone" className="sr-only">
                    Nomor Ponsel Tujuan
                  </label>
                ...
                <div className="mb-6">
                  <label htmlFor="nominalTransfer" className="sr-only">
                    Nominal
                  </label>
                  <div className="relative">
                    <MdAttachMoney
                  ...
                  {modalError && (
                    <p className="text-red-600 text-sm mt-2 text-center animate-fade-in">
                      {modalError}
                    </p>
                  )}
                </div>
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 rounded-lg font-semibold text-gray-700 border border-gray-300 transition duration-300 ease-in-out transform ${
                      modalLoading
                        ...
                        : "bg-white hover:bg-gray-100 active:scale-98 shadow-sm"
                    }`}
                  >
                    Batal
                  </button>
